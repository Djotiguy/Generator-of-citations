// **************** PROJECT 3 : Générateur de citations *****************/
...
// Création de la fonction qui va nous retourner un numero aléatoire
function randomNumbers(max) {
    return Math.floor(Math.random() * Math.floor(max));
}

// Création du gestion d'event sur le boutons
btn.addEventListener('click', () => {

    do{ // Cette boucle assigne notre fonction à la variable ainsi que la taille de notre tableau
        randomNumber = randomNumbers(citations.length) // renvoi un chiffre entre 0 et la taille max de notre tableau
    } while(randomNumber == last); // tant que que nos variables sont égales

    // Assignation de notre variable nombreAleatoire à nos éléments citation & auteur.
    citation.textContent = citations[randomNumber][0]; // récupère le premier élément du tableau
    auteur.textContent = citations[randomNumber][1]; // récupère le second élément du tableau
    dernier = randomNumber; // Afin de nous retourner a chaque fois un random
});